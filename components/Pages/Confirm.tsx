        ...
        <div className=" max-w-screen-xl mx-auto">
          ...
          >
              <dl className="grid grid-cols-3 lg:items-center justify-center gap-5" >
                ...
                <InputLabel title="名前" id="name" type="text" placeholder="山田太郎" required={true} getValues={values.name} />
                <InputLabel title="会社名 or 屋号名" id="company" type="text" placeholder="例）株式会社リ・スターツ" required={true} getValues={values.company} />
                <InputLabel title="メールアドレス" id="email" type="email" placeholder="例）info@restarts.co.jp" required={true} getValues={values.email} />
                <InputLabel title="電話番号" id="tel" type="tel" placeholder="例）042-866-3979" required={true} getValues={values.tel} />

                <SelectCheckboxLabel title="お問い合わせの種類" id="select" required={true} getValues={values.select}  />

                <TextAreaLabel title="内容" id="description" placeholder="例）お問い合わせ内容を入力してください" required={false} getValues={values.description} />
